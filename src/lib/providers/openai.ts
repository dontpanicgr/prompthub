...
export class OpenAIProvider implements ProviderClient {
  ...
  async suggest(args: SuggestArgs): Promise<SuggestResponse> {
    const { text, variant, model = 'gpt-4o-mini', temperature = 0.3, maxTokens = 1000 } = args
    
    const systemPrompt = this.getSystemPrompt(variant)
    
    try {
      const response = await this.http.request<OpenAIResponse>('/chat/completions', {
        method: 'POST',
        headers: {
      ...
      throw new ProviderError(
        `OpenAI suggest failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'openai'
      )
    }
  }
  ...
  async chat(args: ChatArgs): Promise<ChatResponse> {
    const { messages, model = 'gpt-4o-mini', temperature = 0.7, maxTokens = 2000, stream = false } = args
