...
export function Tooltip({
    ...
    timeoutRef.current = window.setTimeout(() => {
      ...
      switch (placement) {
        case "top":
          ...
          tooltipLeft = left + width / 2 + window.scrollX
          break
        case "bottom":
          ...
          break
        case "left":
          tooltipTop = top + height / 2 + window.scrollY
  ...
  const tooltipNode = isOpen && coords ? (
    <div
      role="tooltip"
      id="tooltip"
      style={{ position: "absolute", top: coords.top, left: coords.left, transform: placement === "top" || placement === "bottom" ? "translate(-50%, -100%)" : placement === "left" ? "translate(-100%, -50%)" : "translate(0, -50%)", pointerEvents: "none", zIndex: 50 }}
      className={cn(
